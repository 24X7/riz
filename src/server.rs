...
pub fn build_app(state: Arc<AppState>) -> AxumRouter {
    // Public, autogenerated OpenAPI 3.1 of the function route table — the HTTP
    // twin of the MCP tool list, served like /health and the agent card.
    ...
    let app = AxumRouter::new()
        ...
        .route(
            "/openapi.json",
            get(move || {
                let s = openapi_state.clone();
                async move {
                    Json(crate::system::openapi::openapi_document(&s.riz_state).await)
                        .into_response()
                }
            }),
        )
