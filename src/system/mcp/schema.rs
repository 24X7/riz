...
pub(super) fn tool_input_schema(routes: &[String], mcp: Option<&McpToolConfig>) -> serde_json::Value {
    ...
    let mut route_prop = serde_json::json!({
        "type": "string",
        "description": "Optional \"METHOD /path\" selector when the function declares multiple routes. Omit to use the first declared route."
    });
    if routes.len() > 1 {
        route_prop["enum"] = serde_json::json!(routes);
    }
    properties.insert("route".into(), route_prop);

    // body — declared schema verbatim, else the generic string.
    let body_prop = match mcp.and_then(|m| m.body.clone()) {
        Some(schema) => schema,
    ...
    properties.insert("body".into(), body_prop);
    ...
    // params stay accepted (HTTP query strings are open-world).
    let mut qp = serde_json::json!({
        "type": "object",
    ...
    if let Some(m) = mcp {
        if !m.query.is_empty() {
            ...
            qp["properties"] = serde_json::Value::Object(qprops);
            if !qrequired.is_empty() {
                qp["required"] = serde_json::json!(qrequired);
    ...
    // require a param that only one route uses).
    ...
    if all_params.is_empty() {
        ...
        let mut pp = serde_json::json!({
            "type": "object",
            "properties": serde_json::Value::Object(pprops),
            "additionalProperties": false
        });
        if !required_everywhere.is_empty() {
            pp["required"] = serde_json::json!(required_everywhere);
    ...
    if !top_required.is_empty() {
        schema["required"] = serde_json::json!(top_required);
    }
    schema
}
