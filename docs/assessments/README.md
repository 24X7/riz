...
| `/_riz/v1/*` (LLM gateway) + `/cache/invalidate` mounted as bare axum routes — bypass bearer auth; README overclaims gating | **Run 3** | **Open — top priority** (security-grade; the endpoint that spends money is unauthenticated) |
| Flagship WASM proofs (broker, guards) skip in CI — wasip1 target never installed; claims_truth verifies the proof fn *exists*, not that it *ran* | **Run 3** | **Open** — CI must build the wasm fixtures (or claims_truth must fail on skipped proofs) |
