//! Cross-runtime error status code pass-through parity — Slice H.
//!
//! For each shipped runtime, fires GET /echo?status=NNN with several
//! non-2xx codes and asserts riz passes the handler's statusCode through
//! to the HTTP client unchanged. Verifies the response-shaping path in
//! `server.rs::gateway_to_axum` faithfully forwards 4xx and 5xx.
//!
//! Codes tested: 400, 404, 418 (teapot), 500, 503. Includes a 200
//! control to prove the default path also works.
//!
//! Run: `cargo nextest run --test runtime_parity_errors`
...
async fn exercise_error_status_codes(addr: SocketAddr, function_name: &str) {
    let client = reqwest::Client::new();
    let ready_url = format!("http://{addr}/echo");
    wait_for_ready(&client, &ready_url).await;

    // 200 control + all the codes we care about.
    for status in &[200u16, 400, 404, 418, 500, 503] {
        let url = format!("http://{addr}/echo?status={status}");
        let resp = client.get(&url).send().await.expect("send");
        assert_eq!(
            resp.status().as_u16(),
            *status,
            "{function_name}: ?status={status} must pass through; got {} (body = {:?})",
            resp.status(),
...
#[tokio::test]
async fn bun_echo_passes_error_status_codes() {
    if !bun_available() {
...
"#
    ...
    exercise_error_status_codes(addr, "echo-bun").await;
}

#[tokio::test]
async fn node_echo_passes_error_status_codes() {
    if !node_available() {
...
"#
    ...
    let addr = boot_riz(&config_toml).await;
    exercise_error_status_codes(addr, "echo-node").await;
}

#[tokio::test]
async fn python_echo_passes_error_status_codes() {
...
"#
    ...
    let addr = boot_riz(&config_toml).await;
    exercise_error_status_codes(addr, "echo-python").await;
}

#[tokio::test]
async fn rust_echo_passes_error_status_codes() {
...
"#,
    ...
    let addr = boot_riz(&config_toml).await;
    exercise_error_status_codes(addr, "echo-rust").await;
}
